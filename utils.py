...
def spectral_clustering(affinity_matrix_, n_clusters, k, seed=1, n_init=20):
    affinity_matrix_ = check_symmetric(affinity_matrix_)  # 保证为对称正方形矩阵
    random_state = check_random_state(seed)
    # print(11111111111)
    laplacian = sparse.csgraph.laplacian(affinity_matrix_, normed=True)  # 归一化后的拉普拉斯矩阵D**-0.5*L*D**-0.5
    # print(222222222222222222)
    _, vec = sparse.linalg.eigsh(sparse.identity(laplacian.shape[0]) - laplacian,
                                 k=k, sigma=None, which='LA')
    # 运行速度慢，可以试试下面这句
    # _, vec = scipy.linalg.eigh(sparse.identity(laplacian.shape[0]) - laplacian, eigvals=(laplacian.shape[0] - k, laplacian.shape[0] - 1))
    # _, vec = np.linalg.eig(sparse.identity(laplacian.shape[0]) - laplacian)
